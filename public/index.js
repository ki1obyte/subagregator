// Ждем, пока объект window станет доступен
if (typeof window !== 'undefined') {

    // Вся наша старая логика теперь внутри этой функции
    const initializeAggregator = () => {
        ...
        // Проверяем, существуют ли элементы. Если нет, ждем и пробуем снова.
        if (!cardsContainer || !filterByDate || !qrModal) {
            setTimeout(initializeAggregator, 100); // Попробовать снова через 100 мс
            return;
        }
        ...
        async function loadSubscriptions() {
            try {
                const response = await fetch('/data.json'); // Путь к data.json теперь от корня
