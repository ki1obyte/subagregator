# check_proxies.py (версия для параллельного запуска)
...
import socket
import socks
import sys # Импортируем sys для чтения аргументов

# --- ИЗМЕНЕНИЕ 1: Функция теперь читает прокси из локального файла ---
...
# Функции parse_vless, setup_xray, check_proxy остаются БЕЗ ИЗМЕНЕНИЙ
# (скопируйте их из вашей последней рабочей версии)
...
def check_proxy(vless_url, parsed):
    ...
    if parsed['security'] in ['tls', 'reality']:
        ...
        if parsed['fp']: tls_settings["utls"] = {"enabled": True, "fingerprint": parsed['fp']}
        if parsed['security'] == 'reality' or parsed['pbk']:
            stream_settings["security"] = "reality"
            stream_settings["realitySettings"] = {"show": False, "fingerprint": parsed['fp'] or "chrome", "serverName": parsed['sni'], "publicKey": parsed['pbk'], "shortId": parsed['sid'], "spiderX": parsed['spx']}
        else: stream_settings["tlsSettings"] = tls_settings
    ...
    config = {"log": {"loglevel": "warning"}, "inbounds": [{"port": 10808, "listen": "127.0.0.1", "protocol": "socks"}], "outbounds": [{"protocol": "vless", "settings": {"vnext": [{"address": parsed['address'], "port": parsed['port'], "users": [{"id": parsed['id'], "encryption": "none", "flow": parsed['flow']}]}]}, "streamSettings": stream_settings}]}
    max_retries = 10; retry_delay = 5
    for attempt in range(max_retries):
        print(f"Attempt {attempt + 1}/{max_retries}...")
        config_path, proc = '', None
        try:
            with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f: json.dump(config, f, indent=2); config_path = f.name
            ...
            if proc.poll() is not None:
                stdout, stderr = proc.communicate(); print(f"Xray process failed to start. Stderr: {stderr.strip()}"); return False
            socks.set_default_proxy(socks.SOCKS5, "127.0.0.1", 10808); socket.socket = socks.socksocket
            start_time = time.time(); response = requests.get('http://httpbin.org/ip', timeout=30); end_time = time.time()
            if response.status_code == 200: print(f"SUCCESS: Proxy is working. Response time: {end_time - start_time:.2f}s"); return True
            else: print(f"FAILURE on attempt {attempt + 1}: Proxy responded with status code {response.status_code}")
        except Exception as e: print(f"FAILURE on attempt {attempt + 1}: An error occurred: {e}")
        finally:
            if proc:
                ...
                if stderr: print(f"Xray stderr: {stderr.strip()}")
            if os.path.exists(config_path): os.unlink(config_path)
            socks.set_default_proxy(); time.sleep(1)
        if attempt < max_retries - 1: print(f"Waiting for {retry_delay} seconds before retrying..."); time.sleep(retry_delay)
    print("All attempts failed for this proxy."); return False

# --- ИЗМЕНЕНИЕ 2: Основная логика теперь работает с файлом ---
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python check_proxies.py <input_file>")
        sys.exit(1)
    ...
    # Имя выходного файла тоже можно передавать аргументом, но для простоты оставим test.txt
